...
public sealed class ResetPasswordCommandHandler : ICommandHandler<ResetPasswordCommand, Success>
{
    ...
    public ResetPasswordCommandHandler(IPasswordHashService passwordHashService, IResponseCacheService responseCacheService, IRepositoryBase<User, Guid> userRepository, IRepositoryBase<AuthProvider, Guid> authRepository, IUnitOfWork unitOfWork)
    {
    ...
    public async Task<Result<Success>> Handle(ResetPasswordCommand command, CancellationToken cancellationToken)
    {
        ...
        // Bước 2: Kiểm tra Otp
        var cachedData = await _responseCacheService.GetCacheResponseAsync($"{AuthConstants.ResetPasswordOtpCachePrefix}{userFound.PhoneNumber}");
        ...
        var newPasswordHash = _passwordHashService.HashPassword(command.Password);
        ...
        await _responseCacheService.DeleteCacheResponseAsync(
            $"{AuthConstants.ResetPasswordOtpCachePrefix}{userFound.PhoneNumber}");

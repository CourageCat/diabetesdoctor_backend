    ...
    : RepositoryBase<Conversation>(context), IConversationRepository
{
    ...
    public async Task<UpdateResult> ToggleGroupVisibilityAsync(IClientSessionHandle session, ObjectId conversationId, UpdateDefinition<Conversation> update,
        CancellationToken cancellationToken = default)
    {
        var filter = Builders<Conversation>.Filter.Eq(conversation => conversation.Id, conversationId);
        var options = new UpdateOptions { IsUpsert = false };
        return await DbSet.UpdateOneAsync(session, filter, update, options, cancellationToken);
    }
}
...
public sealed class ConversationChangedDomainEventHandler(
    ...
    : 
        ...
        IDomainEventHandler<ConversationLinkedEvent>
        
{
    public async Task Handle(ConversationCreatedEvent notification, CancellationToken cancellationToken)
    {
        var integrationEvent = new ConversationCreatedIntegrationEvent
        {
            ...
            ConversationType = notification.ConversationType.ToEnum<ConversationType, ConversationTypeEnum>(),
    ...
    {
        var integrationEvent = new ConversationDeletedIntegrationEvent
        {
            ConversationId = notification.ConversationId
        };
        var @event = OutboxEventExtension.ToOutboxEvent(kafkaSettings.Value.ConversationTopic, integrationEvent);
    ...
    public async Task Handle(ConversationLinkedEvent notification, CancellationToken cancellationToken)
    {
        var integrationEvent = new ConversationLinkedIntegrationEvent
        {
            ConversationId = notification.ConversationId,
            ConsultationId = notification.ConsultationId,
            IsOpened = notification.IsOpened,

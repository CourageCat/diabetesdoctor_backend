...
using NotificationService.Contract.Exceptions;
...
public class KafkaSubscriberBase<T> : BackgroundService
{
    ...
    private readonly KafkaSettings _kafkaSettingses;
    ...
    public KafkaSubscriberBase(
        ...
        IIntegrationEventFactory integrationEventFactory, KafkaSettings kafkaSettingses)
    {
        _consumer = consumer;
        _options = options;
        ...
        _logger = logger;
        _kafkaSettingses = kafkaSettingses;
    }
            ...
            if (_options.AcceptEvent(@event))
            {
                while (retries <= _options.ServiceRetries)
                {
                    ...
                    catch (DomainException ex)
                    {
                        ...
                        var dlqEvent = OutboxEventExtension.ToOutboxEvent(_kafkaSettingses.DeadTopic, @event);
                    ...
                    catch (Exception ex)
                    {
                        ...
                        if (retries > _options.ServiceRetries)
                        {
                            ...
                            {
                                var dlqEvent = OutboxEventExtension.ToOutboxEvent(_kafkaSettingses.DeadTopic, @event, messageValue.RetryCount);
                                await outboxRepo.SaveAsync(dlqEvent, stoppingToken);
                                _logger.LogWarning("Moved to DLQ: {msg}", ex.Message);
                                return;
                            }
                            var retryEvent = OutboxEventExtension.ToOutboxEvent(_kafkaSettingses.RetryTopic, @event, messageValue.RetryCount + 1);

...
public class RepositoryBase<TEntity> : IRepositoryBase<TEntity>, IDisposable where TEntity : DomainEntity<ObjectId>
{
    ...
    public async Task<BulkWriteResult<TEntity>> ReplaceManyAsync(IClientSessionHandle session, IEnumerable<TEntity> entities, CancellationToken cancellationToken = default)
    {
        ...
        return await DbSet.BulkWriteAsync(session, writes, cancellationToken: cancellationToken);
    }
    ...
    public async Task<UpdateResult> SoftDeleteAsync(IClientSessionHandle session, ObjectId id, CancellationToken cancellationToken = default)
    {
        ...
        var builder = Builders<TEntity>.Update
            .Set(x => x.ModifiedDate, CurrentTimeService.GetCurrentTimeUtc())

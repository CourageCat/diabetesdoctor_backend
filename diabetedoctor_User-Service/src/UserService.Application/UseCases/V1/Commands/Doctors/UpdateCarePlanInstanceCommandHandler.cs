...
public sealed class UpdateCarePlanInstanceCommandHandler : ICommandHandler<UpdateCarePlanInstanceCommand, Success>
{
    ...
    public UpdateCarePlanInstanceCommandHandler(IRepositoryBase<PatientProfile, Guid> patientRepository, IRepositoryBase<DoctorProfile, Guid> doctorRepository, IRepositoryBase<CarePlanMeasurementInstance, Guid> carePlanMeasurementInstanceRepository)
    {
    ...
    public async Task<Result<Success>> Handle(UpdateCarePlanInstanceCommand command, CancellationToken cancellationToken)
    {
        ...
        {
            return Result.Failure<Success>(CarePlanInstanceErrors.CanNotUpdateCarePlanInstanceBelongToPatient);
        }
        ...
        var duplicatedInstance = await _carePlanMeasurementInstanceRepository.FindSingleAsync(
            instance => instance.PatientProfileId == patientFound.Id && instance.RecordType == recordType &&
                        instance.ScheduledAt == command.ScheduledAt && instance.DoctorProfileId == doctorFound.Id, true, cancellationToken);

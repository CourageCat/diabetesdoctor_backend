...
public sealed class CreateHbA1cValueCommandHandler
    : ICommandHandler<CreateHbA1cValueCommand, Success<string>>
{
    ...
    public CreateHbA1cValueCommandHandler(IRepositoryBase<PatientProfile, Guid> patientProfileRepository, IRepositoryBase<HealthRecord, Guid> healthRecordRepository, IPublisher publisher, IUnitOfWork unitOfWork)
    {
    ...
    public async Task<Result<Success<string>>> Handle(CreateHbA1cValueCommand command, CancellationToken cancellationToken)
    {
    ...
    /// Tạo HealthRecord
    /// </summary>
    private static HealthRecord CreateHealthRecord(Guid patientId, CreateHbA1cValueCommand command)
    {
        var value = HbA1cValue.Of(command.Value);
        ...
        var record = HealthRecord.Create(patientId, RecordType.HbA1c, value, measuredAt: measurementAt, personNote: command.PersonNote, id: recordId);

...
public sealed class CreateBloodPressureValueCommandHandler
    : ICommandHandler<CreateBloodPressureValueCommand, Success<string>>
{
    ...
    public CreateBloodPressureValueCommandHandler(IRepositoryBase<PatientProfile, Guid> patientProfileRepository, IRepositoryBase<HealthRecord, Guid> healthRecordRepository, IPublisher publisher, IUnitOfWork unitOfWork)
    {
    ...
    public async Task<Result<Success<string>>> Handle(CreateBloodPressureValueCommand command, CancellationToken cancellationToken)
    {
        var patientProfile = await _patientProfileRepository.FindSingleAsync(p => p.UserId == command.UserId, true, cancellationToken);
    ...
    /// Tạo HealthRecord
    /// </summary>
    private static HealthRecord CreateHealthRecord(Guid patientId, CreateBloodPressureValueCommand command)
    {
        var value = BloodPressureValue.Of(command.Systolic, command.Diastolic);
        ...
        var record = HealthRecord.Create(patientId, RecordType.BloodPressure, value, measuredAt: measurementAt, personNote: command.PersonNote, id: recordId);
        return record;
    }

...
public sealed class CreateHeightValueCommandHandler
    : ICommandHandler<CreateHeightValueCommand, Success<string>>
{
    ...
    public CreateHeightValueCommandHandler(IRepositoryBase<PatientProfile, Guid> patientProfileRepository, IRepositoryBase<HealthRecord, Guid> healthRecordRepository, IPublisher publisher, IUnitOfWork unitOfWork)
    {
    ...
    public async Task<Result<Success<string>>> Handle(CreateHeightValueCommand command, CancellationToken cancellationToken)
    {
    ...
    /// Tạo HealthRecord
    /// </summary>
    private static HealthRecord CreateHealthRecord(Guid patientId, CreateHeightValueCommand command)
    {
        var value = HeightValue.Of(command.Value);
        ...
        var record = HealthRecord.Create(patientId, RecordType.Height, value, measuredAt: measurementAt, id: recordId);

    ...
    : IQueryHandler<GetCarePlanMeasurementQuery, Success<List<CarePlanInstanceResponse>>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        DoctorProfile? doctorFound = null;
        if (query.DoctorId != null)
        {
            doctorFound = await _context.DoctorProfiles.FirstOrDefaultAsync(x => x.UserId == query.DoctorId.Value, cancellationToken);
        }
        var rawInstances = await _context.CarePlanMeasurementInstances
            ...
            .Where(x =>
                ...
                (doctorFound != null ? 
                    (x.DoctorProfileId == doctorFound.Id) 
                    : x.DoctorProfileId == null) &&

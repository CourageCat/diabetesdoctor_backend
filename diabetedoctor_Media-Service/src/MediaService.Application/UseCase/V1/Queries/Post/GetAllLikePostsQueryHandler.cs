...
public sealed class GetAllLikePostsQueryHandler(IMongoDbContext context) : IQueryHandler<GetAllLikePostsQuery, Success<PagedList<PostResponse>>>
{
    public async Task<Result<Success<PagedList<PostResponse>>>> Handle(GetAllLikePostsQuery request, CancellationToken cancellationToken)
    {
        ...
        var postsLikedFound = await GetAllLikePostsAsync(filterExpression, filter.UserId, filter.CategoryIds, request.SortType, (request.SortType.Equals("likedDate") && cursorValues.Count != 0) ? DateTime.Parse(cursorValues[0]) : null, request.IsSortAsc, request.PageSize, cancellationToken);
        var hasNext = postsLikedFound.Count > request.PageSize;
        var nextCursor = "";

        if (hasNext)
        {
            postsLikedFound.RemoveRange(request.PageSize, postsLikedFound.Count - request.PageSize);
            var lastPostFromBookMark = postsLikedFound.ToList()[^1];
        ...
        var result = PagedList<PostResponse>.Create(postsLikedFound.ToList(), request.PageSize, nextCursor, hasNext);
        return Result.Success(new Success<PagedList<PostResponse>>(
            PostMessage.GetAllLikePostsSuccessfully.GetMessage().Code,
            PostMessage.GetAllLikePostsSuccessfully.GetMessage().Message, result));
    }
    ...
    private string GetSortCursorValue(PostResponse post, string sortType)
    {
        var result = sortType switch
        {
            ...
            "likedDate" => post.LikedDate != null ? post.LikedDate.Value.ToString("o") : "",
            _ => post.Id.ToString(),
        };
        return result;
    }
    ...
    private async Task<List<PostResponse>> GetAllLikePostsAsync(FilterDefinition<Domain.Models.Post> filterExpression, string? userId, ObjectId[]? categoryIds, string sortType, DateTime? cursorLikedDate, bool isSortAsc, int pageSize, CancellationToken cancellationToken)
    {
        var postProjection = new BsonDocument()
        {
            ...
            { "is_liked", true },
            { "word_count", 1 },
            { "liked_date", "$likedpost.created_date" }
        };
        ...
        var documents = await context.Posts.Aggregate()
            ...
            {
                { "likedpost", new BsonDocument("$ne", new BsonArray()) }
            }))
            // Join
            ...
            .Sort(sortType.Equals("likedDate") ? new BsonDocument("likedpost.created_date", isSortAsc ? 1 : -1) : sortDefinition)
            // Filter
            .Match(filterExpression)
            .AppendStage<BsonDocument>(cursorLikedDate != null ? new BsonDocument("$match", new BsonDocument
            {
                { "likedpost.created_date", new BsonDocument(isSortAsc ? "$gt" : "$lt", cursorLikedDate) }
            }) : new BsonDocument("$match", new BsonDocument()))
            .Limit(pageSize + 1)
            // Add Field IsLiked and assign true value
            .AppendStage<BsonDocument>(new BsonDocument("$addFields",
                new BsonDocument("is_liked", true)
            ))
            // Check if User has bookmarked post or not
            .AppendStage<BsonDocument>(new BsonDocument("$lookup", new BsonDocument
            {

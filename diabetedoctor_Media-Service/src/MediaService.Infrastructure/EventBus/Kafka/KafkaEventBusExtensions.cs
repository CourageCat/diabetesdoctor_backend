using MediaService.Contract.Settings;
using MediaService.Infrastructure.EventBus.Kafka.EventSubscribers;
using MediaService.Infrastructure.Options;

namespace MediaService.Infrastructure.EventBus.Kafka;

public static class KafkaEventBusExtensions
{
    public static IHostApplicationBuilder AddKafkaProducer(this IHostApplicationBuilder builder, KafkaSettings kafkaSettingses, string connectionName = "kafka")
    {
        builder.AddKafkaProducer<string, EventEnvelope>(connectionName,
            configureSettings: (settings) =>
            {
                settings.Config.BootstrapServers = kafkaSettingses.BootstrapServer;
                ...
                settings.Config.SaslUsername = kafkaSettingses.SaslUsername;
                settings.Config.SaslPassword = kafkaSettingses.SaslPassword;
    ...
    private static IHostApplicationBuilder AddKafkaMessageEnvelopConsumer(this IHostApplicationBuilder builder, KafkaSettings kafkaSettingses,
    ...
    public static IHostApplicationBuilder AddKafkaEventConsumer<T>(this IHostApplicationBuilder builder, KafkaSettings kafkaSettingses,
        Action<ConsumerWorkerOptions>? configureOptions = null, string connectionName = "kafka") where T : class
    {
        ...
        builder.AddKafkaMessageEnvelopConsumer(kafkaSettingses, options.KafkaGroupId, connectionName);
        ...
        builder.Services.AddHostedService<KafkaSubscriberBase<T>>(sp =>
        {
            ...
            return new KafkaSubscriberBase<T>(consumer, options, logger, scopeFactory, integrationEventFactory, kafkaSettingses);
        });
        return builder;
    }
